...
class FaceRestoreHelper(object):
    ...
    def get_face_landmarks_5(self,
        ...
        with torch.no_grad():
            ...
            # ic(bboxes.shape)
            # cv2.imwrite('resized_image.jpg', input_img)
    ...
    def align_warp_face(self, save_cropped_path=None, border_mode='constant'):
        ...
        for idx, landmark in enumerate(self.all_landmarks_5[-1: ]):
    ...
    def paste_faces_to_input_image(self, save_path=None, upsample_img_list=None, draw_box=False, face_upsampler=None):
        h, w, _ = self.input_img.shape
        h_up, w_up = int(h * self.upscale_factor), int(w * self.upscale_factor)

        results = []

        for i, upsample_img in enumerate(upsample_img_list):
            if upsample_img is None:
                # simply resize the background
                # upsample_img = cv2.resize(self.input_img, (w_up, h_up), interpolation=cv2.INTER_LANCZOS4)
                upsample_img = cv2.resize(self.input_img, (w_up, h_up), interpolation=cv2.INTER_LINEAR)
                raise Exception("upsample img couldn't be none")
            else:
            ...
            restored_face, inverse_affine = self.restored_faces[i], self.inverse_affine_matrices[i]
